<template>
  <div class="panel" v-loading="loading">
    ...
    <el-table :data="projects" row-key="id" stripe height="400">
    ...
    <!-- Deliverable Dialog -->
    <el-dialog v-model="deliverableDialog" title="交付物" width="540px">
      <el-form :model="deliverableForm" label-width="100px">
        <div class="form-grid">
          ...
          <el-form-item label="阶段"><el-input v-model="deliverableForm.phase" /></el-form-item>
          <el-form-item label="负责人"><el-input v-model="deliverableForm.owner" /></el-form-item>
    ...
    <el-dialog v-model="riskDialog" title="风险登记" width="600px">
      <el-form :model="riskForm" label-width="100px">
        <div class="form-grid">
          ...
          <el-form-item label="负责人"><el-input v-model="riskForm.owner" /></el-form-item>
...
import {
  createProject, createProjectDeliverable, createProjectRisk, createProjectTemplate, deleteProject, deleteProjectDeliverable,
  deleteProjectRisk, deleteProjectTemplate, getProjects, getProjectTemplates, updateProject, updateProjectDeliverable,
  updateProjectRisk, updateProjectTemplate,
} from '../api'
...
const deliverableForm = ref<any>({ name: '', deliverable_type: '', phase: '', owner: '', status: '待处理', due_date: '', description: '' })
...
function openCreate() { editingId.value = null; form.value = { project_no: '', name: '', product_model: '', phase: '概念', progress: 0, owner: '', start_date: '', end_date: '', risk_level: '低' }; dialogVisible.value = true }
function openEdit(row: any) { editingId.value = row.id; form.value = { project_no: row.project_no, name: row.name, product_model: row.product_model, phase: row.phase, progress: row.progress, owner: row.owner, start_date: row.start_date, end_date: row.end_date, risk_level: row.risk_level }; dialogVisible.value = true }
async function save() {
...
function openCreateDeliverable(project: any) { deliverableProjectId.value = project.id; deliverableEditingId.value = null; deliverableForm.value = { name: '', deliverable_type: '', phase: '', owner: '', status: '待处理', due_date: '', description: '' }; deliverableDialog.value = true }
function openEditDeliverable(project: any, d: any) { deliverableProjectId.value = project.id; deliverableEditingId.value = d.id; deliverableForm.value = { ...d }; deliverableDialog.value = true }
...
function openCreateRisk(project: any) { riskProjectId.value = project.id; riskEditingId.value = null; riskForm.value = { risk_type: '技术', description: '', impact: '中', probability: '中', severity: '中', owner: '', status: '待处理', mitigation: '' }; riskDialog.value = true }
