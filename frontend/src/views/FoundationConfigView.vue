<template>
  <div class="panel" v-loading="loading">
    <div class="toolbar">
      <div>
        <strong>基础配置</strong>
        <span class="muted"> · 生命周期、枚举字典统一维护</span>
      </div>
...
const childDialogTitle = computed(() => `${childEditingId.value ? '编辑' : '新增'}状态`)
const tabName = computed(() => ({ lifecycle: '生命周期', dictionary: '字典项' }[activeTab.value] || '配置'))

async function loadRows() {
  const [lifecycleRows, dictionaries] = await Promise.all([
